...
const AdvantageChart: React.FC<LineChartProps> = ({ data }) => {
  ...
  return (
    <svg viewBox={`0 0 ${viewBoxWidth} ${viewBoxHeight}`} width="100%" height="100%">
      {/* Title */}
      <text x={viewBoxWidth / 2} y={padding / 2} textAnchor="middle" fontSize="16" fill="black">The advance of the game:</text>
      ...
      <line x1={padding} y1={padding + chartHeight / 2} x2={padding + chartWidth} y2={padding + chartHeight / 2} stroke="gray" strokeWidth="1" />

      {/* Vertical lines at data points */}
      {points.map((point, index) => (
        <line key={index} x1={point.x} y1={padding} x2={point.x} y2={padding + chartHeight} stroke="gray" strokeWidth="1" />
      ))}
