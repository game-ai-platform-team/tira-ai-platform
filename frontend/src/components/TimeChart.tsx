...
const TimeChart: React.FC<LineChartProps> = ({ data }) => {
  ...
  const yValues = Array.from({ length: Math.floor(maxDataValue / yInterval) + 1 }, (_, index) => index * yInterval);
  ...
  return (
    <svg viewBox={`0 0 ${viewBoxWidth} ${viewBoxHeight}`} width="100%" height="100%">
      {/* Title */}
      <text x={viewBoxWidth / 2} y={padding / 2} textAnchor="middle" fontSize="16" fill="black">Time Taken</text>

      {/* Box around the chart */}
      <rect x={padding} y={padding} width={chartWidth} height={chartHeight} fill="none" stroke="black" strokeWidth="2" />

      {/* Horizontal lines */}
      {yValues.map((value, index) => (
        <line key={index} x1={padding} y1={viewBoxHeight - padding - (value / maxDataValue) * chartHeight} x2={viewBoxWidth - padding} y2={viewBoxHeight - padding - (value / maxDataValue) * chartHeight} stroke="gray" strokeWidth="1" />
      ))}
      ...
      {points.map((point, index) => (
        <text key={index} x={point.x} y={point.y - 10} textAnchor="middle" fill="blue">{data[index]}</text>
      ))}
    </svg>
  );
};
