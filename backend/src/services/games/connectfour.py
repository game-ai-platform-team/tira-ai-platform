...
class ConnectFour:
    ...
    def drop_piece(self, column):
        if not self.game_over:
            if self.board[self.rows - 1][column] == 0:  # Check if the bottom row of the column is empty
    ...
    def check_win(self, row, column):
        ...
        for c in range(self.columns - 3):
            if self.board[row][c] == self.player and self.board[row][c+1] == self.player and self.board[row][c+2] == self.player and self.board[row][c+3] == self.player:
                return True

        # Check vertical
        for r in range(self.rows - 3):
            if self.board[r][column] == self.player and self.board[r+1][column] == self.player and self.board[r+2][column] == self.player and self.board[r+3][column] == self.player:
                return True
