...
from config import OIDC_CLIENT_ID, OIDC_CLIENT_SECRET, ROOT_DIR
...
@app.route("/")
def index():
    return send_file(ROOT_DIR.parent / "frontend" / "dist" / "index.html")


@app.route("/<path:path>")
def default(path):
    return send_from_directory(ROOT_DIR.parent / "frontend" / "dist", path)
